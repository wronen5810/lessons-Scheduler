...
export default function TeacherDashboard() {
  const { settings, loading } = useTeacherSettings();
  ...
  const cards: FeatureCard[] = [
    ...
    {
      label: 'Slots',
      description: 'Configure recurring & one-time slots',
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {cards.map((card) => (
          <Link
            key={card.href}
            href={card.href}
            className={`group flex flex-col items-center justify-center gap-3 p-6 rounded-2xl border-2 transition-all ${card.color} shadow-sm hover:shadow-md`}
          >
